...
Autres choix de modelisation :
  ...
  - Dispersion  : repulsion douce a longue portee, distincte de la separation
                  qui reste une contrainte de securite a courte portee.
...
class Params:
    ...
    g_spread: float = 1.0  # dispersion (longue portee, exploration seule)
    g_rally: float = 1.6
    ...
    # --- 13. pheromones ---------------------------------------------------------------------- #
    dopa_deposit: float = 8.0
    dopa_diffuse: float = 0.20  # 1/s, coefficient de diffusion
    dopa_tau: float = 40.0  # s, constante de temps d'evaporation
    dopa_attract: float = 2.0  # poids dans le score de ciblage
...
class World:
    ...
    def __init__(self, bounds: pygame.Rect, p: Params, rng: random.Random):
        ...
        self.cx = np.repeat(xs[:, None], self.rows, axis=1)
        self.cy = np.repeat(ys[None, :], self.cols, axis=0)
...
class Drone:
    ...
    def _w_spread(self, nb, g: float) -> Vector2:
        """
        Dispersion : repulsion douce sur toute la portee de perception, active
        en phase d'exploration uniquement. Elle etale l'essaim pour couvrir plus
        de terrain, la ou la separation ne fait qu'empecher les collisions.
        """
        p = self.params
        if g <= 0.0:
            return Vector2(0, 0)
        ...
        for other, d, _w in nb:
            ...
            cnt += 1
        if cnt == 0 or push.length_squared() < 1e-12:
            return Vector2(0, 0)
...
class Swarm:
    ...
    def _knowledge_gap(self) -> float:
        ...
        individual = float(self.conf.mean())
        return max(0.0, collective - individual)

    def _advance_phase(self, forced: bool = False) -> None:
        p = self.params
        nxt = PHASE_CYCLE[self.phase]
        if self.phase == "explore":
            self.last_gap_before = self._knowledge_gap()
    ...
    def _choose_target(self, d: Drone, conf, dopa, cort) -> Vector2 | None:
        ...
        Cellule maximisant
            ...
            besoin x (1 + k_D.dopamine - k_C.cortisol) x biais_de_cap
        ...
        attract = np.clip(1.0 + p.dopa_attract * dopa - p.cort_repel * cort, 0.0, None)
        ...
        score = (
            ...
            * attract
            * bias
            * np.exp(-dist / max(1.0, p.target_horizon))
            / np.sqrt(dist)
        )
    ...
    def step(self, dt: float) -> None:
        ...
        if self.frame % 10 == 0:
            ...
            self.conf_gap = self._knowledge_gap()
...
def draw_phase_banner(screen, font_big, font, swarm: Swarm, W: int) -> None:
    ...
    txt = (
        ...
        f"reste {swarm.phase_remaining():4.1f} s   "
...
def draw_hud(
    ...
    header = [
        ...
        (f"ecart de connaissance {m['gap'] * 100:5.2f} %", (240, 150, 35)),
        (
    ...
    lh = 15
    n_lines = len(header) + len(TUNABLES) + len(ALL_BEHAVIORS) + 2
    h = min(H - 16, 18 + lh * n_lines)
...
def main(headless: bool = False, max_frames: int = 0, seed: int | None = None) -> None:
    ...
    while running:
        ...
        for ev in pygame.event.get():
            if ev.type == pygame.QUIT:
                ...
                if ev.key in (pygame.K_ESCAPE, pygame.K_q):
                    ...
                    swarm._advance_phase(forced=True)
        ...
        if params.rally_mode == "schedule":
            for k, pt in enumerate(w.rv_points):
                col = C_RV if k == swarm.cycle % len(w.rv_points) else (70, 62, 48)
