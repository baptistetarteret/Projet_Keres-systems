...
BOUCLE DE MISSION EN TROIS PHASES
  1. EXPLORATION  : l'essaim se disperse et ratisse. La portee radio (45 m) est
     tres inferieure a l'espacement moyen des drones (~110 m a 20 drones sur
     10 ha), donc l'essaim se fragmente et chacun accumule une connaissance
  ...
  3. PARTAGE      : bref maintien en formation, puis redispersion. Les drones
     ...
     d'autres les attirent, y compris a longue distance. Les cellules revenues
     avec une confiance PARTIELLE (< seuil) restent attractives, ce qui produit
     naturellement des retours cibles sur les zones d'interet.
...
REPARTITION EN EVENTAIL A LA DISPERSION
  ...
  Ce biais s'attenue exponentiellement (fan_tau) : il ouvre l'eventail au
  demarrage puis s'efface pour laisser la carte decider.
...
COUPLAGE DE LA DOPAMINE  (parametre `dopa_saturate`, touche D)
  ...
  du taux de detection, et ralentissement du scan. Deux reponses possibles :
    - LINEAIRE (defaut) : l'effet est proportionnel a la concentration, qui
      n'est pas bornee. Sur une grappe de 4 cibles la concentration atteint ~32,
      d'ou une attirance x65, un taux de detection x81 et une vitesse ramenee a
      8 % de la consigne. L'essaim converge alors massivement sur les foyers
      decouverts et s'y attarde : c'est un emballement par retroaction positive
      (plus on ralentit, plus on detecte, plus on depose), conserve ici parce
      qu'il est precisement l'objet d'etude.
    - SATUREE : on remplace la concentration x par x/(1+x), borne a 1. Les gains
      redeviennent des amplitudes interpretables et l'emballement disparait.
      Pour une amplitude comparable il faut alors remonter les gains d'un ordre
      de grandeur (Attir. dopa ~ 20, Gain dopa ~ 25).
...
drones sont deconnectes. Il doit donc etre deductible sans echange.
  ...
  - "schedule" : suite de points fixee AVANT le decollage a partir de la
                 geometrie de la zone (parcours en boustrophedon). Pre-accordee,
                 donc sans communication. Mode par defaut.
  - "centroid" : barycentre de l'essaim. C'est un ORACLE (chaque drone devrait
                 connaitre la position de tous les autres) ; fourni uniquement
...
Autres choix de modelisation :
  - Entite      : drone a voilure tournante, sature en vitesse, en acceleration
                  ...
                  profil d'arrivee, ralentissement en virage, ralentissement de
  ...
  - Dispersion  : repulsion douce a longue portee, de rayon PROPRE (spread_radius)
                  et non celui de la perception, pour rester active en mode
                  topologique ou les voisins peuvent etre tres eloignes.
                  Distincte de la separation, qui reste une securite a courte
                  portee.
  ...
  - Connaissance: carte de confiance dans [0,1] par cellule.
                  c <- 1 - (1-c) exp(-lambda dt). Plusieurs passages sont
                  necessaires pour valider une cible.
...
horloges synchronisees. Une verite terrain globale, invisible aux drones, sert
uniquement a arreter le chronometre.
...
class PhaseProfile:
    ...
    `mul` multiplie les gains de base definis dans Params. Les gains restent donc
    reglables au clavier, la phase ne faisant que les moduler.
    """
...
PHASES = {
    ...
    # Ralliement : le comportement de ralliement remonte juste apres la securite,
    # la couverture tombe en fin de file (on scrute encore un peu en chemin).
    "rally": PhaseProfile(
        label="RALLIEMENT",
        color=(240, 180, 120),
        order=(
...
class Params:
    ...
    # --- 5. communication ------------------------------------------------------ #
    # Volontairement TRES inferieure a l'espacement moyen en exploration
    # (~110 m a 20 drones sur 10 ha) : sans quoi l'essaim resterait connecte en
    # permanence et la phase de ralliement n'aurait aucun sens.
    comm_radius: float = 45.0  # m
    ...
    spread_radius: float = 70.0  # m, dispersion (longue portee)
    ...
    n_clusters: int = 3
    per_cluster: int = 4
    cluster_spread: float = 16.0  # m, ecart-type du tirage dans une grappe
    n_isolated: int = 4
    ...
    # --- 13. pheromones ---------------------------------------------------------------------- #
    # `dopa_saturate` choisit la reponse aux concentrations : voir l'en-tete.
    ...
    cort_diffuse: float = 0.25
    cort_tau: float = 90.0
    cort_repel: float = 0.8

    # --- 14. ciblage -------------------------------------------------------------------------- #
    # Le ciblage balaie TOUTE la carte du drone (pas une fenetre locale) : c'est
    # ce qui permet, apres un partage, de repartir vers un foyer de dopamine
    # decouvert par un autre a l'autre bout de la zone. L'horizon ci-dessous est
    # une longueur d'attenuation, pas une coupure franche.
    target_horizon: float = 140.0  # m
    coverage_every: int = 8  # reevaluation de la cible 1 frame sur N
    pher_every: int = 6  # diffusion des pheromones 1 frame sur N (voir step)
...
TUNABLES = [
    ...
    ("Dispersion", "g_spread", 0.0, 4.0, 0.1),
    ("R disper. m", "spread_radius", 10.0, 200.0, 5.0),
...
class World:
    ...
    Note : `bounds` est un pygame.Rect, donc a coordonnees entieres. Les
    dimensions de zone doivent rester des entiers en metres.
    """
    ...
    def __init__(self, bounds: pygame.Rect, p: Params, rng: random.Random):
        ...
        # boustrophedon : les rendez-vous successifs balaient la zone, ce qui
        # deplace progressivement le centre d'operations de l'essaim.
    ...
    def _make_rv_points(self, p: Params) -> list[Vector2]:
        ...
        for j in range(ny):
            ...
            for i in cols:
                ...
                y = (j + 0.5) / ny * b.height
                pt = Vector2(x, y)
                # un rendez-vous ne doit pas tomber dans un obstacle
    ...
    def _make_obstacles(self, p: Params, rng: random.Random) -> None:
        b = self.bounds
        tries = 0
        while len(self.obstacles) < int(p.n_obstacles) and tries < 800:
            tries += 1
    ...
    def _make_targets(self, p: Params, rng: random.Random) -> None:
        ...
        for _ in range(int(p.n_clusters)):
            ...
            placed, tries = 0, 0
            while placed < int(p.per_cluster) and tries < 400:
                tries += 1
                ...
                if not self.target_mask[i, j]:
                    ...
                    placed += 1
            if placed == 0:  # grappe impossible : au moins une cible au centre
                self.target_mask[ci, cj] = True
...
class Drone:
    ...
    La vitesse n'est pas constante. Trois mecanismes la font varier : profil
    d'arrivee sur la cible, ralentissement en virage (cosinus de l'erreur de cap),
    et ralentissement de scan en zone dopaminee.
    """
    ...
    def _w_spread(self, nb, g: float) -> Vector2:
        ...
        essaim dilue, aucun des k voisins ne passerait le filtre et la dispersion
        s'eteindrait silencieusement au moment ou elle est le plus utile.
        """
        p = self.params
        if g <= 0.0 or not nb:
            return Vector2(0, 0)
    ...
    def decide(
        ...
        # joue l'essentiel de l'adaptation, un comportement place en fin de file
        # ne recevant que le reliquat laisse par les precedents.
        total = Vector2(0, 0)
...
class Swarm:
    ...
    def _compute_rally_point(self) -> Vector2:
        ...
        if p.rally_mode == "centroid":
            ...
            # autres. Fourni comme borne de comparaison uniquement.
            if not self.drones:
                return w.base_point()
    ...
    def _knowledge_gap(self) -> float:
        ...
        Il croit pendant l'exploration (chacun accumule de son cote) et retombe
        d'un coup au partage. C'est l'indicateur le plus parlant du dispositif.
        """
    ...
    def fan_weight(self) -> float:
        """Amplitude du biais de secteur : maximale a la dispersion, decroissante."""
        p = self.params
    ...
    def _cone_lambda(self, d: Drone, dopa: np.ndarray):
        ...
        au bord du cone, et est multiplie par (1 + gain * reponse(dopamine)) :
        c'est par la que la dopamine accelere la detection dans une zone jugee
        prometteuse.
        """
        p = self.params
        w = self.world
    ...
    def _choose_target(
        ...
        evaluee sur TOUTE la carte propre au drone, et non sur une fenetre locale.
        C'est ce qui permet, apres un partage, de repartir vers un foyer de
        dopamine decouvert par un autre a l'autre bout de la zone. L'exponentielle
        donne une longueur d'attenuation L plutot qu'une coupure franche.
        ...
        pour qu'aucune cible ne soit abandonnee avant validation). Une cellule
        revenue d'un partage avec une confiance PARTIELLE garde donc un besoin
        non nul : elle sera revisitee.
        ...
        Aucun oracle : tout est lu dans la carte du drone. Il peut donc viser une
        cellule qu'un autre a deja couverte si l'information ne lui est pas
        parvenue.
        """
        p = self.params
        w = self.world
    ...
    def step(self, dt: float) -> None:
        ...
        #    coefficient de diffusion cumule (rate * dt_accumule) reste tres en
        #    deca de la limite de stabilite 0,24, et l'evaporation exponentielle
        #    est exacte quel que soit le pas. Gain : le laplacien sur toute la
        #    pile de cartes est le poste le plus couteux du simulateur.
        self._pher_dt += dt
        ...
        if not self.done and int(w.found.sum()) >= w.n_targets:
            # n_targets == 0 (zone trop encombree) compte comme un succes
            # immediat, sans quoi la mission ne pourrait jamais s'achever.
            self.done = True
...
def render_fields(
    ...
    arrondi de 6,4 a 6 px decale la carte de plusieurs dizaines de pixels au
    bord droit, alors que drones, obstacles et cibles sont places avec px().
    """
...
def draw_hud(
    ...
    header = [
        ...
        (
            "dopamine : " + ("saturee" if params.dopa_saturate else "lineaire"),
            C_DOPA,
        ),
        (
...
def main(headless: bool = False, max_frames: int = 0, seed: int | None = None) -> None:
    ...
    pygame.display.set_caption(
        f"Essaim de drones — {ZONE_W_M} x {ZONE_H_M} m, cycles exploration / ralliement"
    )
    clock = pygame.time.Clock()

    # La hauteur de ligne du panneau est deduite de la place disponible : avec
    # 27 reglages et 8 barres de budget, une valeur fixe deborderait de la
    # fenetre. La police suit la hauteur de ligne.
        ...
        if layer == "tout":
            render_fields(
                ...
                [
                    ...
                    (swarm.layer("dopamine", collective), C_DOPA, 0.4, 0.80),
                ],
                w.cols,
                w.rows,
                w.cell,
            ...
            render_fields(
                screen,
                [(swarm.layer(layer, collective), C_DOPA, 0.4, 0.85)],
                w.cols,
                w.rows,
                w.cell,
            )
